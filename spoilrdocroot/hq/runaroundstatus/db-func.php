...
//        require_once "secret.php";
	...
	if (($spoilrdb = mysql_connect(SPOILRDB_SERVER, SPOILRDB_USER, SPOILRDB_PASS)) == FALSE) {
                echo 'Could not connect to database server ' . SPOILRDB_SERVER . '.';
                foot();
                exit(1);
        }
        ...
        // Select spoilr database
        if (mysql_select_db(SPOILRDB_NAME, $spoilrdb) == FALSE) {
                echo 'Could not select spoilr database ' . SPOILRDB_NAME . '.';
                foot();
                exit(1);
        }
	...
	function spoilr_query_db($query)
        {
		global $spoilrdb;
		$mydb = $spoilrdb;
                $result = mysql_query($query, $mydb);
